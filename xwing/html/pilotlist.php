...
$p = $_GET['p'];
  if (is_null($p)){
    header("location: pilotlists.php");
  }
...
<?php
	...
	$pilot = array();
        $sql = "Select pilots.name, pilots.faction, pilots.cost FROM pilots WHERE pilots.id =". $ship;
        $result = mysql_query($sql) or die("Unable to select: ".mysql_error());
        while($row = mysql_fetch_row($result)){
        array_push($pilot, $row[0], $row[1], $row[2]);
        }
	echo "<a href='/pilot.php?p=". $ship ."&d=". $d ."&f=". $f ."'>";
        switch ($pilot[1]){
		case "1":
...
?>
 ...
 <table class="front" style="width: 100%">
  ...
  $squads = array();
if ($f == 0){
  $sql = "select list_id, total from (select list_id, total, (select GROUP_Concat(p.id ORDER BY p.cost desc SEPARATOR ',') from list_builds lb join builds b on b.id = lb.build_id join pilots p on b.pilot_id = p.id where lb.list_id = x.list_id) as pilotlist from (Select list_id, SUM(points * SQRT(attendance) * (-(DATEDIFF(CURDATE(), tournaments.date) - 90)/abs((DATEDIFF(CURDATE(), tournaments.date) - 90)/49.9)+50)) as total from placing join tournaments on placing.tournament_id = tournaments.id group by list_id order by total desc) as x ) as y where y.pilotlist = '". $p ."' order by total desc limit 50;";
}else{
