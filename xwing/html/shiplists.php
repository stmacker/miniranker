...
<body bgcolor="black">
  ...
  <br>
  <div class="details">
Below is a list of the top squads simplified to ship type compostion based on current data, select a squad to see more information on what tournaments it placed in.
<br><br>
...
if ($f == 0){
  ...
  foreach($squads as $key => $squad) {
	...
	$pilot = array();
        $sql = "Select ships.name, ships.faction FROM ships WHERE ships.id =". $key;
        $result = mysql_query($sql) or die("Unable to select: ".mysql_error());
        while($row = mysql_fetch_row($result)){
