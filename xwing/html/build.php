...
<?php
        $pilot = array();
        $sql = "Select pilots.id, pilots.name, builds.upgrades, pilots.cost, pilots.faction, builds.nickname FROM builds join pilots on builds.pilot_id = pilots.id WHERE builds.id =". $b;
	...
	echo "<a href='/pilot.php?p=". $pilot[0] ."&d=". $d ."&f=". $f ."'>";
	switch ($pilot[4]){
		case "1":
	...
	$upgrades = array();
        ...
        foreach($upgrades as $upgrade) {
          ...
          echo "<br><a title='". $upg[2] ."' href='/pilot-builds.php?p=". $pilot[0] ."&u=". $upgrade . "&d=". $d ."&f=". $f ."'><span style='color: #33CCFF;'> + ". $upg[0] ."</span> <span style='color: #CCCCCC;'>(". $upg[1] .")</span></a>";
        }
