...
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
  ...
  <link rel="shortcut icon" href="favicon.ico">
    ...
    <script type="text/javascript" src="armada.js"></script>
</head>
<body bgcolor="black">
...
<?php
    ...
    foreach($ship_count as $key => $ship) {
        $pilot = array();
        $sql = "Select ships.name, builds.upgrades, ships.cost, case when ships.fighter = 1 then 0 else ships.faction end FROM builds join ships on builds.ship_id = ships.id WHERE builds.id =". $key;
        $result = mysql_query($sql) or die("Unable to select: ".mysql_error());
        while($row = mysql_fetch_row($result)){
	...
	$upgrades = array();
	  ...
	  $cost = $cost + $upgr[1];
        ...
        if ($ship > 1) {
          echo "<span style='color: #CCCCCC; font-size: 18px'><b>&nbsp;x". $ship ."</b></span>";
        }
        echo "<span style='color: #CCCCCC; font-size: 18px'> (". $cost .")</span></a>";
        echo " ";
	if ($pilot[3] != 0) {
		echo "<br>";
        }
