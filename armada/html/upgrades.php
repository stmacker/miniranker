...
The values to the left of each upgrade are relative points for better comparison of rankings.
 ...
 <table class="front" style="width: 100%">
  ...
  $upgrades = array();
  if ($f == 0){
    $sql = "Select upgrades.id, SUM(points * SQRT(attendance) * (-(DATEDIFF(CURDATE(), tournaments.date) - ". $d .")/abs((DATEDIFF(CURDATE(), tournaments.date) - ". $d .")/49.9)+50)) as total from upgrades left join builds on find_in_set(upgrades.id, builds.upgrades) left join list_builds on list_builds.build_id = builds.id left join placing on list_builds.list_id = placing.list_id left join tournaments on placing.tournament_id = tournaments.id group by upgrades.id order by total desc;";
  }else{
    $sql = "Select upgrades.id, SUM(case when ships.faction = ". $f ." then 1 else 0 end * points * SQRT(attendance) * (-(DATEDIFF(CURDATE(), tournaments.date) - ". $d .")/abs((DATEDIFF(CURDATE(), tournaments.date) - ". $d .")/49.9)+50)) as total from upgrades left join builds on find_in_set(upgrades.id, builds.upgrades) left join list_builds on list_builds.build_id = builds.id left join placing on list_builds.list_id = placing.list_id left join tournaments on placing.tournament_id = tournaments.id left join ships on builds.ship_id = ships.id where (upgrades.faction = 0 or locate(". $f .", upgrades.faction) > 0) group by upgrades.id order by total desc;";
  }
